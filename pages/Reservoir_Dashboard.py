...
# ============================================================
# Physical post-processing of CNN-LSTM outputs
# Forecasting starts from 2026-01-01. Neural predictions are continuous,
# so operational discharge variables are constrained to non-negative values.
# Raw values are preserved for transparency.
# ============================================================
    df_pred["CNN_LSTM_Inflow_Raw"] = df_pred["Inflow (m^3/s)"]
    df_pred["CNN_LSTM_Discharge_Raw"] = df_pred["Total discharge (m^3/s)"]

    df_pred["Inflow (m^3/s)"] = np.maximum(
        pd.to_numeric(df_pred["Inflow (m^3/s)"], errors="coerce").ffill().bfill(),
        0.0
    )

    df_pred["Total discharge (m^3/s)"] = np.maximum(
        pd.to_numeric(df_pred["Total discharge (m^3/s)"], errors="coerce").ffill().bfill(),
        0.0
    )
    ...
    df_pred["PPO_Inflow"] = np.maximum(
        df_pred["Inflow (m^3/s)"].to_numpy(float),
        0.0
    )
...
export_df = export_df.rename(columns={
    "PPO_Optimized_Outflow": "PPO Optimized Outflow (m³/s)",
    "PPO_Optimized_Storage": "PPO Optimized Storage (Mm³)",
    storage_panel_col: "CNN-LSTM Storage (Mm³)"
})

# ============================================================
# Share reservoir output with floodplain module
# ============================================================
# Keep internal names for floodplain-module transfer
st.session_state["reservoir_output_df"] = df_pred[
    [
        "date",
        "PPO_Inflow",
        "PPO_Optimized_Outflow",
...
if len(df_pred) > 0:
    st.session_state["latest_ppo_outflow"] = float(
    df_pred["PPO_Optimized_Outflow"].iloc[0]
)

st.dataframe(export_df.head(100), use_container_width=True)

st.caption(
    "Operational inflow is the non-negative CNN–LSTM inflow used for PPO operation. "
    "Negative raw neural-network predictions, if produced, are treated as numerical artifacts "
    "and are not passed to reservoir operation or floodplain mapping."
)
